...
class LogWriter {
    LogWriter();
public:
    LogWriter(const wchar_t* lpszTag, Logger& logger) noexcept
        : logger_(logger)
    {
		::wcscpy_s(szTag_, lpszTag);
    }
	// traditional printf like interfaces, for char and wchar_t
    void write(int level, char const* format, ...) noexcept
    {
        va_list args;
    ...
    void write(int level, wchar_t const* format, ...) noexcept
    {
        va_list args;
        va_start(args, format);
        _write(level, format, args);
...
private:
    ...
    class TSafeWriter : public std::basic_ostringstream<charT> {
        TSafeWriter() {}
    public:
        TSafeWriter(LogWriter* logger, int level) noexcept
            : pWriter_(logger), level_(level)
        {}
        ~TSafeWriter()
        { pWriter_->write(level_, __super::str().c_str()); }
        void setLogger(LogWriter* pLogger) noexcept
        { pWriter_ = pLogger; }
    private:
        LogWriter* pWriter_;
        int level_;
...
protected:
    virtual void _write(int level, char const* format, va_list& args) noexcept
    {
    ...
    virtual void _write(int level, wchar_t const* format, va_list& args) noexcept
    {
