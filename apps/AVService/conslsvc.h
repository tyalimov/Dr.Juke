...
public:
    ...
    virtual std::wstring getLogFilename(const wchar_t* lpszServicename) const
    {
        ...
        //    ::wcscat_s(szTemp, L"\\");
        return std::wstring(LR"(C:\Users\tremy\Desktop\)") + lpszServicename + L".log";
    }
    ...
    virtual void onContinue() noexcept
    {
    }

    virtual void onInterrogate() noexcept
    {
    }
...
#endif
    virtual void onUnknownRequest(DWORD /*dwControl*/) noexcept
    {
    }

    void setServiceStatus(DWORD dwState) noexcept
    {
...
//Implementation
protected:
    static void WINAPI _serviceMain(DWORD dwArgc, LPWSTR* lpszArgv) noexcept
    {
        s_pProgram->serviceMain(dwArgc, lpszArgv);
    }
    static DWORD WINAPI _serviceControlHandlerEx(DWORD dwControl, DWORD dwEventType, LPVOID lpEventData, LPVOID lpContext) noexcept
    {
